...
- Add an optional raw serial-over-TCP passthrough mode, disabled by default, that leases the amplifier's serial link to a single external client (for example SPE Expert Controller) over the LAN. While a client is connected the server sends zero serial bytes of its own and forwards bytes verbatim in both directions; on disconnect it reclaims the port and resumes polling automatically. A second concurrent client is rejected rather than queued, and a session refuses to start while an automatic transaction is already driving the amplifier.
- Passively observe the amplifier→client direction during a lease and expose the result with a distinct `passthrough-tap` provenance, so the dashboard and API keep tracking the amplifier without overstating what the server knows. Tapped status is merged for display and becomes stale normally when the external client stops polling, but is refused as authority by overtemperature standby, fan policy, and every other server actuation path. There is no byte injection, no response swallowing, and no command/reply correlation.
- Refuse a passthrough session while automatic fan control or overtemperature standby is armed. The connection is closed without a byte being written, because the raw socket carries the amplifier's stream and nothing else: the client sees a clean EOF, or `ECONNRESET` if it wrote first. The reason, naming each control that must be disarmed, reaches the operator through the server log and `GET /api/v1/raw-passthrough` rather than on the socket. Server-side automatic controls are never silently suspended for the duration of a lease and never automatically restored afterwards; an operator who chooses passthrough does so knowing those controls are unavailable.
- Report passthrough state at `GET /api/v1/raw-passthrough`, including which armed controls are currently blocking a session, whether tapped telemetry is still fresh, and whether the server currently owns the serial port.
...
- Report `automaticControlsAvailable` as true while the passthrough listener is idle and the server owns the serial port. It previously read false in every state, including when passthrough was disabled outright. It describes port ownership only: true does not mean a control is enabled or armed, which is what `blockedByArmedControls` reports.
- Expire tapped status back to display-derived state once the external client stops refreshing it. A tapped `0x90` outranked newer display telemetry for as long as the lease lasted, however old it had become, and then took its freshness from the display timestamp -- so `/api/v1/status` and `/api/v1/alarms` could report a stale tapped temperature as `provenance: "passthrough-tap"` with `recentContact: true` while a newer display-derived reading sat underneath it. This is the "becomes stale normally" behavior the passthrough entry above already claimed; it is now true. A live tap is still preferred, and `status-poll` is unaffected -- it is refreshed by the server's own polling, whose age `recentContact` already reports truthfully. Actuation was never involved: fan policy, overtemperature standby and menu debug refuse tapped provenance outright.
- Refuse `POST /api/v1/settings` with HTTP 409 when it would arm automatic fan control or overtemperature standby while a raw passthrough client holds the serial port. The update was previously accepted and persisted, leaving a control that read back as armed while the active lease refused every serial write it would have made. The refusal is decided inside the same boundary that claims the port, so an update can no longer race session setup and commit "armed" into a session being established at that moment. Disarming is never blocked, and no other setting is affected.
- Stop a scheduled poll from wedging passthrough lease acquisition. `BeginRawPassthrough` held `writeMu` while retiring the live session and waiting for the read loop to unwind, but a read loop that has reached a scheduled poll needs that same mutex before it can unwind -- so the two waited on each other until the retire timed out and refused a client that should have been admitted. The transition is now marked under `writeMu` and the mutex released before anything waits: the parked poll acquires it, observes the flag, fails closed, and that failure is what ends the session the retire is waiting for.
- Subscribe `/api/v1/status/ws` before sending its first payload. The handler resolved and sent, then subscribed, so an authority transition published in that gap -- which spans a JSON marshal and a socket write -- reached nobody, and with a static display nothing else ever woke the socket. A client that connected as a lease began could serve the wrong authority for the life of the connection.
- Treat `rawPassthroughEnabled` and `rawPassthroughListenAddress` as restart-requiring in the settings reply. The passthrough controller is built once from the startup snapshot, so changing either updates what is persisted and nothing about what is running; the reply previously said only "settings saved".
- Explain why raw passthrough is unavailable instead of reporting a bare "disabled". `GET /api/v1/raw-passthrough` now names the setting that won, and reports `enabled: false` with `listenerAvailable: false` and the bind error when the listener could not bind, rather than describing an enabled listener no client can reach. `enabled` is documented as configured *and running*, so a listener that never bound was never enabled; `listenerAvailable` and `note` are what distinguish it from a passthrough the operator turned off.
- Reject `rawPassthroughEnabled` together with `pollingMode: "off"` at `POST /api/v1/settings` with HTTP 400. No serial source is created with polling off, so passthrough had nothing to lease: the combination saved cleanly and then reported itself unavailable. It is refused at the point it would be created, and documented as a prerequisite in the README and the served OpenAPI. A configuration already on disk still starts, deliberately — a conflict that one request can fix should not become a boot failure that takes overtemperature standby with it — and is explained by `note` as before.
