...
- Explain why raw passthrough is unavailable instead of reporting a bare "disabled". `pollingMode: "off"` stops the serial source being created at all, so an enabled passthrough with a configured serial port silently got nothing to lease. `GET /api/v1/raw-passthrough` now names the setting that won, and reports `enabled: false` with `listenerAvailable: false` and the bind error when the listener could not bind, rather than describing an enabled listener no client can reach. `enabled` is documented as configured *and running*, so a listener that never bound was never enabled; `listenerAvailable` and `note` are what distinguish it from a passthrough the operator turned off.
