...
- Refuse a passthrough session with a clear `409` while automatic fan control or overtemperature standby is armed, naming each control that must be disarmed. Server-side automatic controls are never silently suspended for the duration of a lease and never automatically restored afterwards; an operator who chooses passthrough does so knowing those controls are unavailable.
- Report passthrough state at `GET /api/v1/raw-passthrough`, including which armed controls are currently blocking a session and whether tapped telemetry is still fresh.
